       ...
       <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-60 text-sm shadow-lg'>
        ...
        <div className='w-full'> 
         ...
         <button className='bg-primary text-white w-full py-2 rounded-md text-base'>{state === 'Sign Up' ? "Create Account" : "Login"}</button>
         {
          ...
          state === "Sign Up" 
          ? <p>Alread have an account?  <span onClick={()=>setstate('Login')}  className='text-primary underline cursor-pointer'>Login Here</span></p> :
           <p>Created an new account? <span onClick={()=>setstate('Sing Up')} className='text-primary underline cursor-pointer'> Click Here</span></p>
         }
